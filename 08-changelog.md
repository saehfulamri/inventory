...
- Fase 5 — TASK-110: migrasi modul Penerimaan ke Inertia — halaman `Pages/Purchases/{Index,Create,Show}.vue`; form create dengan baris item dinamis (tambah/hapus baris, subtotal & grand total reaktif, auto-fill harga beli dari produk); finalisasi via dialog konfirmasi; util baru `formatDate` & `todayISO`; view Blade `purchases/*` dihapus; test `PurchaseReceivingTest` & `PurchaseAuthorizationTest` dialihkan ke `assertInertia`.
- Fase 5 — TASK-111: migrasi modul Penjualan ke Inertia — halaman `Pages/Sales/{Index,Pos,Show}.vue`; POS interaktif (pencarian produk tetap via endpoint JSON `GET /sales/products` yang sudah ada, cart reactive dengan clamp stok, perhitungan kembalian live, submit via `useForm transform`); struk dengan cetak (`window.print()`); view Blade `sales/*` dihapus; test `SaleManagementTest` & `SaleAuthorizationTest` dialihkan ke `assertInertia`.
- Fase 5 — TASK-112: migrasi modul Laporan ke Inertia — halaman `Pages/Reports/{Index,Sales,Stock,Movements,Purchases}.vue` + util baru `formatNumber`; link "Unduh CSV" tetap ke endpoint streaming server-side `GET /reports/sales/export`; sekaligus memperbaiki 2 bug lama di view: (1) kartu ringkasan `reports.index` selalu menampilkan 0 karena blade mereferensikan variabel yang tidak pernah dikirim (`$sales`/`$purchases`/`$stock`/`$movements`), kini memakai ringkasan asli (`salesSummary`, `purchaseSummary`, `stockSummary`, total unit bergerak dari `movementTotals`); (2) filter keyword & status stok di `reports.stock` tidak berpengaruh karena `filters()` tidak menangkap `keyword`/`low_stock` — keduanya kini diteruskan dan didukung test regresi (total 178 test); view Blade `reports/*` dihapus; `ReportFeatureTest` dialihkan ke `assertInertia`.
- Fase 6 — TASK-113: sweep akhir migrasi Phase 12 — seluruh view Blade modul lama dihapus (`auth/login.blade.php`, `layouts/app.blade.php`, `welcome.blade.php`); tersisa hanya root template `app.blade.php`; seluruh test halaman GET sudah memakai assertion `assertInertia` (tidak ada `assertSee`/`assertDontSee` pada halaman GET yang tersisa); verifikasi `npm run build` OK.
- Fase 6 — TASK-114: hardening CSP — semua script inline aplikasi dihapus: konfigurasi rute Ziggy tidak lagi disuntik lewat `@routes` + `<script>` inline di blade, melainkan modul statis `resources/js/ziggy.js` (hasil `php artisan ziggy:generate`) yang di-import `app.js` dan diteruskan ke plugin `ZiggyVue` (`use(ZiggyVue, ziggyConfig)`); `'unsafe-inline'` dihapus dari `script-src`. Temuan penting saat review (gejala layar beku/putih): **root cause utamanya bukan data-page Inertia, melainkan mode development — saat `npm run dev` aktif, Vite menulis `public/hot` dan seluruh aset & HMR dilayani dari origin dev (default `http://127.0.0.1:5173`); `script-src 'self'` memblokir script lintas-origin itu sehingga JS tidak pernah jalan dan Vue tidak mounting.** Perbaikan: **dua lapis** — (1) `SecurityHeaders` membaca `public/hot` dan, bila dev server aktif, menambahkan origin-nya (plus `ws://` untuk HMR) ke `script-src`/`style-src`/`img-src`/`font-src`/`connect-src`; (2) **CSP nonce per-respons sebagai defense-in-depth** untuk data-block Inertia `<script type="application/json" data-page>` (yang tetap tunduk `script-src` di sebagian browser) — middleware membangkitkan nonce, `View::share('cspNonce', …)`, header memakainya, dan direktif `@inertia` di-override di `AppServiceProvider` agar skrip data-page memakai `nonce="…"` yang sama. Hasil akhir: `script-src 'self' 'nonce-…'` (+ origin dev saat `npm run dev`), tanpa `'unsafe-inline'`, `style-src` tetap `'self' 'unsafe-inline'`; terverifikasi di Chrome headless: halaman login berhasil di-mount oleh Vue. Catatan: script logger konsol yang disuntik tooling dev lokal (Boost) terblokir CSP (nonce tidak disuntikkan tooling) — hanya noise konsol, tidak mempengaruhi fungsi app. Tinjauan CSRF/XSRF Inertia: alur baku dipertahankan — cookie `XSRF-TOKEN` dikirim balik sebagai header `X-XSRF-TOKEN` oleh klien Inertia baku (tanpa axios custom), `VerifyCsrfToken` default; NFR-009/010 (aksesibilitas & atribut `lang` pada `app.blade.php`) tidak berubah. `SecurityHeadersTest` diperkuat: nonce di header harus sama dengan nonce skrip data-page di HTML, plus test bahwa origin Vite dev server diizinkan saat `public/hot` ada.
